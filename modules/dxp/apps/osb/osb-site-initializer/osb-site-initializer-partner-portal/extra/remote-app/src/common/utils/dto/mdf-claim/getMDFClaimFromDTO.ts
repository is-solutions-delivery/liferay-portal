...
export function getMDFClaimFromDTO(
	mdfClaim: MDFClaimDTO,
	mdfClaimStatus: LiferayPicklist,
	mdfRequestId: number
): MDFClaim {
	return {
		...mdfClaim,
		activities: mdfClaim.mdfClaimToMdfClaimActivities?.map((activity) => ({
			...activity,
			budgets: activity.mdfClaimActivityToMdfClaimBudgets,
		})),
		mdfClaimStatus,
		r_mdfRequestToMdfClaims_c_mdfRequestId: mdfRequestId,
		totalClaimAmount: mdfClaim?.amountClaimed,
		totalrequestedAmount: mdfClaim.mdfRequestedAmount,
	};
}
