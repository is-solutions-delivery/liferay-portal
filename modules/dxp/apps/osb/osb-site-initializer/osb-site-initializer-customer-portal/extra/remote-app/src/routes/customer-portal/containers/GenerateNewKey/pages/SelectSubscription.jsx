...
const SelectSubscription = ({
	...
	const instanceSizeOfTheSelectedKeys = state.activationKeys?.map((item) => {
		return item.sizing.replace(/sizing/i, '');
	});
	...
	const submitKey = useCallback(async () => {
		...
		const nameOfSelectedProducts = [...new Set(licenseEntryTypes)]
			.join(', ')
			.toLowerCase();

		const objetoCorrespondente = selectedSubscription.licenseKeyEndDates.find(
			(item) => item.licenseEntryType.includes(nameOfSelectedProducts)
		);
		const endDateRelacionada = objetoCorrespondente
			? objetoCorrespondente.endDate
			: null;
				...
				try {
					const createKeyPromises = state.activationKeys.map(
						async (item) => {
							const licenseKey = {
								accountKey,
								expirationDate: endDateRelacionada,
