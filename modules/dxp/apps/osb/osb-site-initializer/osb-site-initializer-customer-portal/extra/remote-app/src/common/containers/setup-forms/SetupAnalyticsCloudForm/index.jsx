...
const SetupAnalyticsCloudPage = ({
	...
	const [addHighPriorityContactList, setAddHighPriorityContact] = useState(
		[]
	);
	const [
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (featureFlags.includes('LPS-159127')) {
				...
				const actLiferayContact = (items, fn) =>
					Promise.all(
						items?.map((item) => fn(item, project, client))
					);

				await actRaysourceContact(removeContactRole);
				await actRaysourceContact(associateContactRole);
