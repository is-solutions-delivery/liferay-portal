...
const COMMENT_REPLY = Liferay.Language.get('comment-reply');
const QUESTION = Liferay.Language.get('question');
const ANSWER = Liferay.Language.get('answer');
...
export default function UserActivityQuestionRow({
	...
	const rowFields = () => {
		...
		if (question.labelAnswers[1] === 'RE:') {
			storeFields.setLabel = COMMENT_REPLY;
			storeFields.setSymbol = 'reply';
		} else if (question.labelAnswers[0] === 'RE:') {
			storeFields.setLabel = ANSWER;
			storeFields.setSymbol = 'message';
		} else {
			storeFields.setLabel = QUESTION;
			storeFields.setSymbol = 'question-circle-full';
		}
			...
			<div className="align-items-center d-flex flex-wrap justify-content-between">
				<ul className="align-items-center c-mb-2 d-flex flex-nowrap list-badges list-unstyled stretched-link-layer">
					{getQuestionCreatedInDays <= DAYS_UNTIL_SHOW_LABEL &&
						rowFields().setLabel !== COMMENT_REPLY && (
							<li>
					...
					<li>
						<QuestionBadge
							...
							symbol={rowFields().setSymbol}
							value={rowFields().setLabel}
						/>
					</li>
