...
const PaymentMethod = () => {
				...
				options: [
					{
						...
						id: 0,
						orderItem: {
							discountAmount: productPromo,
							finalPrice: productDiscounted,
							quantity: 1,
							skuId: fullPriceSKU.id,
							unitPrice: productDiscounted,
						},
						skuType: SKU.pay_in_full,
	...
	useEffect(() => {
		if (!orderId) {
			createOrder(accountId, channel.id, skus[0].id, product).then(
				(response) =>
					dispatch({
