...
const PolicyDetail = ({
	...
	const [showPanel, setShowPanel] = useState<Array<boolean>>([]);
	...
	const sortedByData = (a: InfoPanelType, b: InfoPanelType) => {
		if (a.date > b.date) {
			return -1;
		}
		if (a.date < b.date) {
	...
	const arraySortedByDate = arrayOfHistory.sort(sortedByData);
	...
	return (
		<div className="h-100 padding pb-6 policy-detail-container rounded">
					...
					arraySortedByDate?.map(
						(item: InfoPanelType, index: number) => {
							return (
								<>
									<div
										className="bg-neutral-0 flex-row gsdc-panel history-detail-border p-6"
										key={index}
									>
										<div>
											<div className="data-panel-hide float-left w-25">
												{item.date}
											</div>
											<Panel
												Description={() =>
											...
											>
												<div className="layout-show-details ml-auto mt-4 pl-3 pr-3 pt-3 w-75">
