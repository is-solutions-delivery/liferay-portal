...
const TableListMobile: React.FC<Props> = ({headers, rows}) => {
	...
	const displayHistoryPanel = (index: number) => {
		...
		if (selectedRow === showPanel[index].valueOf()) {
			supportArray[index] = !supportArray[index];
			setShowPanel(supportArray);
		}
		else {
			const arrayDefault = new Array(showPanel.length).fill(false);
			arrayDefault[index] = !arrayDefault[index];
			setShowPanel(arrayDefault);
		}
	};

	useEffect(() => {
		const supportArray = dataActivities.map(() => {
			return false;
		});
								...
								return (
									<Row key={index}>
										...
										>
											...
											>
												<div className="bg-message d-flex font-collapse m-2 mr-3 p-2 rounded">
													{row.message}
												</div>
											</Panel>
										</div>
