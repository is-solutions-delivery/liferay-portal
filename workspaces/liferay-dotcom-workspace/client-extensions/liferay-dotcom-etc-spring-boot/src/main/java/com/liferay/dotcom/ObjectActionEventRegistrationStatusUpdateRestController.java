...
@RestController
public class ObjectActionEventRegistrationStatusUpdateRestController
	...
	public ResponseEntity<String> post(
		...
		long eventID = eventRegistrationResponseJSONObject.getLong(
			"r_eventRegistration_c_eventId");

		JSONObject eventResponseJSONObject = new JSONObject(
			get(
				"Bearer " + jwt.getTokenValue(),
				StringBundler.concat(
					"/o/c/events/", eventID, "?fields=eventCapacityStatus")));
	...
	private JSONObject _getPayloadJSONObject(JSONObject eventStatusJSONObject) {
		JSONObject eventResgistrationStatusJSONObject = new JSONObject(
		).put(
			"key", eventStatusJSONObject.getString("key")
		).put(
			"name", eventStatusJSONObject.getString("name")
		);

		return new JSONObject(
		).put(
			"eventRegistrationStatus", eventResgistrationStatusJSONObject
		);
	}
