...
class JiraAuth {
	...
	public async getCloudId(token: string) {
		...
		const assessibleResource = data.find(({ name }) => name === JIRA_APP_NAME);
	...
	public async getTokenAndCloudId(userId: string) {
		const state = this.AUTHORIZE_PARAMS.state + userId;

		let cachedCloudID = cacheInstance.get<string>(Cache.KEYS.JIRA_APP_ID_KEY);
		let cachedUserState = cacheInstance.get<JiraAuthorizePayload>(state);

		if (!cachedUserState) {
			const testrayOAuthJira = await this.testray.getOAuthJira(userId);
		...
		if (isTokenExpired) {
			logger.warning(`The token ${state} is expired, generating a new one`);
