...
public class Query {
	 ...
	 * Invoke this method with the command line:
	 *
	 * curl -H 'Content-Type: text/plain; charset=utf-8' -X 'POST' 'http://localhost:8080/o/graphql' -d $'{"query": "query {testrayExportCaseResultTestrayBuild(testrayBuildId: ___, testrayProjectId: ___, testrayRoutineId: ___){}}"}' -u 'test@liferay.com:test'
	 */
	@GraphQLField
			...
			testrayCaseResultResource ->
				testrayCaseResultResource.
					getTestrayExportCaseResultTestrayBuild(
						testrayBuildId, testrayProjectId, testrayRoutineId));
	}
