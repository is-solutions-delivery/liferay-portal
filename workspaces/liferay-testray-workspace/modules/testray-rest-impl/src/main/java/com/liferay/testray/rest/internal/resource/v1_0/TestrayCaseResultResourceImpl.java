...
public class TestrayCaseResultResourceImpl
	...
	public Response getTestrayExportCaseResultTestrayBuild(
			Long testrayBuildId, Long testrayProjectId, Long testrayRoutineId)
		throws Exception {
		...
		Collection<TestrayCaseResult> results = page.getItems();

		return _buildCSVResponse(
			results, testrayBuildId, testrayProjectId, testrayRoutineId);
	}

	private Response _buildCSVResponse(
		Collection<TestrayCaseResult> results, Long testrayBuildId,
		...
		StreamingOutput streamingOutput = outputStream -> {
			...
			try (CSVPrinter csvPrinter = new CSVPrinter(
				...
				for (TestrayCaseResult result : results) {
					String[] runName = result.getTestrayRunName(
					).split(
						"\\|"
					);
					...
					String caseResultUrl =
						baseURI.getScheme() + "://" + baseURI.getAuthority() +
							"/#/project/" + testrayProjectId + "/routines/" +
								testrayRoutineId + "/build/" + testrayBuildId +
									"/case-result/" +
										result.getTestrayCaseResultId();

					csvPrinter.printRecord(
						result.getTestrayCaseName(),
						result.getTestrayCaseTypeName(), result.getPriority(),
						result.getTestrayTeamName(),
						result.getTestrayComponentName(),
						result.getTestrayRunNumber(), runName[0], runName[1],
						runName[2], runName[3], runName[4],
						result.getUserName(), result.getStatus(),
						result.getIssues(), result.getError(),
						result.getComment(), caseResultUrl);
				}
