				...
				value -> new TestrayCaseResult() {
					{
						try {
							...
							comment = GetterUtil.getString(
								value.get("comment_"));
							error = GetterUtil.getString(value.get("errors_"));
							issues = GetterUtil.getString(value.get("issues_"));
							priority = GetterUtil.getLong(
								value.get("priority_"));
							status = GetterUtil.getString(
							...
							if (Validator.isNotNull(value.get("uuid_"))) {
								...
								userName = fullNameGenerator.getFullName(
							...
							if (Validator.isNotNull(value.get("portraitId"))) {
								userImgUrl = UserConstants.getPortraitURL(
									"/image", true,
