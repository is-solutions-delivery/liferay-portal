...
export default class HeadlessAdminUser {
	...
	static async deleteRoleAccountUserAccount(
		roleId: number | string,
		userId: number | string
	) {
		return fetcher.delete(
			`/o/headless-admin-user/v1.0/roles/${roleId}/association/user-account/${userId}`
		);
	}
	...
	static async sendRoleAccountUserAccount(
		roleId: number | string,
		userId: number | string
	) {
		return fetcher.post(
