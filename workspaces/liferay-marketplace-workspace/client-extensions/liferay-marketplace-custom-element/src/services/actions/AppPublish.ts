...
import { getPriceListByCatalogName, getProductById, postPriceEntryIdTierPrice, postPriceListEntry } from '../../utils/api';
...
export default class AppPublish extends BaseAppPublish {
	...
	async syncLicensing(product: Product) {
		...
		const skusByType = (_product?.skus || []).reduce((acc, sku) => {
		...
		for (const currencyCode of currencies) {
			...
			const priceList = await getPriceListByCatalogName(producta?.catalog?.name);

			const priceListResponse = priceList.items.filter((item: any) => item.currencyCode === currencyCode);
	
			if (priceListResponse.length === 0) {
				console.warn(`No price list found for currency: ${currencyCode}`);
				continue;
			}
			...
			for (const licenseTier of licenseTiers) {
				...
				for (const sku of tierSkus) {
					...
					const priceEntryResponse = await postPriceListEntry(priceListId, priceEntry);
	
					await processTier(priceEntryResponse, currencyCode, licenseTier);
				}
			}
		}
