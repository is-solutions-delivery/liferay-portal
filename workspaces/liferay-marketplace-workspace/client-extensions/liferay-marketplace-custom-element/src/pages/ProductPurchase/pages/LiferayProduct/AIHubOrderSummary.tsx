...
import {NewProductPurchaseAIHub} from '../../services/NewProductPurchaseAIHub';
...
const AIHubOrderSummary = () => {
	...
	useEffect(() => {
		if (selectedAccount?.id) {
			const result = zodSchema.newAIHubForm.safeParse(form);

			if (!result.success) {
				navigate('/new-ai-hub-form');
			}
		}
	}, [form, navigate, selectedAccount?.id]);

	useEffect(() => {
		if (addresses && addresses.length > 0 && !paymentStore.billingAddress?.name) {
	...
	const onSubmit = async (
		form: z.infer<typeof zodSchema.newAIHubForm>
	) => {
		const productPurchase = new NewProductPurchaseAIHub(
			selectedAccount,
			product
		);
	...
	return (
		<ProductPurchase.Shell
			className="product-purchase-summary select-payment-step ai-hub-order-summary"
			footerProps={{
				backButtonProps: {
				...
				continueButtonProps: {
					...
					onClick: () =>
						onSubmit(
							form as z.infer<typeof zodSchema.newAIHubForm>
						),
				},
			}}
			subtitle={
		...
		>
			...
			<Section className="ai-hub-summary" label="Billing Address">
				<div 
					className="ai-hub-summary-infomation-card"
				>
					<ClayIcon
						className="mr-3"
						color="#0B5FFF"
			...
			<Section className="ai-hub-summary" label="Payment Method">
				<div className="ai-hub-alert-card">
					...
					<p className="ai-hub-alert-card-text m-0">
						{i18n.translate('this-purchase-will-be-billed-under-your-existing-payment-agreement-the-payment-method-cannot-be-changed-and-no-online-payment-is-required')}
					</p>
				</div>
				...
				>
					...
					<div>
						...
						<div className="sub-text">
							{i18n.translate('offline-payments-using-the-invoice')}
						</div>
					</div>
				</div>
			</Section>
