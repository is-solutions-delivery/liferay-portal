	...
	const productHasTrialSKU = (skus: SKU[]) => {
		skus.forEach(async (sku) => {
			const licenseUsageType = sku?.skuOptions.find((option) => {
				return (
					option?.key === "trial" && option?.value === "yes" && option?.key
				);
			});
			if (
				licenseUsageType &&
				licenseUsageType?.key.toLowerCase() === SkuOptions.TRIAL.toLowerCase()
			) {
				setHasTrial(true);
			}
		});
	...
	const getProductBasePrice = async (product: Product) => {
		product &&
			product.skus.forEach((sku) => {
				const licenseUsageType = sku?.skuOptions.find((option) => {
					return (
						option?.key === "standard" && option?.value === "yes" && option?.key
					);
				});
				if (
					licenseUsageType &&
