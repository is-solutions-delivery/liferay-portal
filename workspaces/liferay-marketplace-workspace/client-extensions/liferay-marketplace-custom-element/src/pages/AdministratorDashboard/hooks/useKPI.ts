...
const partnerShipIntegrationFilter = new URLSearchParams({
	...
	filter: new SearchBuilder()
		.lambdaContains(
			'specificationValues',
			PRODUCT_TECHNOLOGY_INTEGRATION.TECHNOLOGY_PARTNERSHIP
		)
		.build(),
...
const connectorQuartelyReleaseFilter = new URLSearchParams({
	...
	filter: baseSearchBuilder
		.clone()
		.and()
		.lambda('categoryNames', ProductCategorieTypes.PAYMENT_METHODS)
		.build(),
	nestedFields: 'productSpecifications',
	pageSize: '-1',
});
...
const lowCodeConfigurationsPublishedFilter = new URLSearchParams({
	...
	filter: new SearchBuilder()
		.lambda('categoryNames', ProductCategorieTypes.FRAGMENTS)
		.build(),
	nestedFields: 'productSpecifications',
	pageSize: '-1',
});
