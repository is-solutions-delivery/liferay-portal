...
const zodSchema = {
	...
	dxpFree: z.object({
		...
		companyName: z.string().min(3, 'Company name is required').optional().or(z.literal('')),
		...
		jobTitle: z.string().min(3, 'Job title is required').optional().or(z.literal('')),
		notifyMeAboutProducts: z.boolean(),
		purpose: z.string().min(3, 'Purpose is required'),
		purposeOther: z.string().min(3, 'Purpose is required').optional().or(z.literal('')),
		termsAndConditions: z.boolean().refine((data) => data === true),
		userAgreement: z.boolean().refine((data) => data === true),
	}),
