...
(function () {
	function attachEventListeners(container, swiper) {
	...
	function adjustUIForSingleSlide(
		...
		const elementsToHide = [
			nextButton,
			prevButton,
			fragment.querySelector('.carousel-nav-container-indicators'),
		];
	...
	function cloneSlidesForLoop(slides, slideCount, wrapper) {
		const fragment = fragment.createDocumentFragment();
	...
	function setupCarousel() {
		const carouselContainer = fragment.querySelector(
		...
		const liveRegion = fragment.querySelector('.carousel-live-region');
		const nextButton = fragment.querySelector('.carousel-nav-button-next');
		const prevButton = fragment.querySelector('.carousel-nav-button-prev');
		const swiperWrapper =
		...
		function updateActiveBullet() {
			const bullets = fragment.querySelectorAll(
