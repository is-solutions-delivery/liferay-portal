...
const fragmentSearchElements = {
    ...
    searchSuggestionItemTemplate: fragmentElement.querySelector('.suggestions template'),
    searchSuggestions: fragmentElement.querySelector('.search-suggestions'),
    searchSuggestionsInput: fragmentElement.querySelector('.search-suggestions-input'),
    seeAllResultsLink: fragmentElement.querySelector('.search-suggestions-see-all-results-text'),
    suggestions: fragmentElement.querySelector('.suggestions'),
};

const menuElements = {
    menuButton: document.querySelector('.cta-menu-hamburguer'),
    menuIconLines: document.querySelectorAll('.nav-items-menu-button-icon-line'),
...
function getBreadcrumbFromURL(url) {
    ...
    return url.split(' ').map((word, i) =>
        excludeWords.includes(word) && i !== 0 ? word : word.charAt(0).toUpperCase() + word.slice(1)
    ).join(' ');
}
            ...
            if (data?.items?.[0]) {
                ...
                items.suggestions.forEach((suggestion) => {
                    const suggestionLink = document.importNode(
                        fragmentSearchElements.searchSuggestionItemTemplate.content.querySelector('a'),
                        true
                    );
                    const assetURL = suggestion.attributes.assetURL.replace(/\?.*$/, '');
                    suggestionLink.href = assetURL;
                    const suggestionTitle = suggestionLink.querySelector('.search-suggestion-item-title');
                    suggestionTitle.textContent = suggestion.text;
                    const suggestionContent = suggestionLink.querySelector('.search-suggestion-item-content');
                    let contentText = suggestion.attributes.assetSearchSummary;

                    if (contentText) {
                        contentText = contentText.substring(0, 500);
                        suggestionContent.innerHTML = contentText.replace(searchTermRegExp, `<b>$1</b>`);
                    }

                    const suggestionURL = suggestionLink.querySelector('.search-suggestion-item-link');
                    suggestionURL.textContent = getBreadcrumbFromURL(assetURL);
                    fragmentSearchElements.searchSuggestions.appendChild(suggestionLink);
                    fragmentSearchElements.suggestions.classList.add('search-results-found');
                });
            } else {
                fragmentSearchElements.suggestions.classList.remove('search-results-found');
            }
            fragmentSearchElements.suggestions.classList.remove('loading-search', 'search-error');
        })
        .catch(() => {
...
window.addEventListener('load', () => {
    if (!navigation?.default?.DropdownProvider) {return;}
...
window.addEventListener('keyup', (event) => {
    ...
    if (
        ...
        inputElements.indexOf(document.activeElement.tagName.toLowerCase()) === -1
    ) {
        searchInput.focus();
        if (siteSearchWrapper.classList.contains('search-open')) {
