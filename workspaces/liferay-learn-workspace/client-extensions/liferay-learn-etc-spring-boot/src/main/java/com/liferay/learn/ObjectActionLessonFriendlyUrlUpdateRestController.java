...
public class ObjectActionLessonFriendlyUrlUpdateRestController
	...
	public void handleCourseModuleUpdate(
			...
			get(
				...
				UriComponentsBuilder.fromPath(
					...
					StringBundler.concat(
						objectType.equals("P2S3_COURSE") ?
							"r_p2s3CourseToP2S3Modules_c_p2s3CourseId" : "id",
						" eq '", objectEntryId, "'")
				).queryParam(
					"nestedFields",
		...
		int index = objectType.equals("P2S3_COURSE") ? 0 : 1;
		...
		for (int i = 0; i < objectEntryChildrenJSONArray.length(); i++) {
			...
			if (modchildModuleJSONObjectule.has("p2s3ModuleToP2S3Quizzes")) {
	...
	public ResponseEntity<String> post(
		...
		if (objectDefinitionExternalReferenceCode.equals("P2S3_COURSE") ||
			...
			_putObjectEntry(
				...
				2,
				objectType.equals("P2S3_LESSON") ? "/o/c/p2s3lessons/" :
					"/o/c/p2s3quizzes/");
		}
		else {
